...
var WebClient = function(io) {
  ...
  //report status
  this.socket.on('webreport', function(timeData,countData) {
    $("#stats")
     .find(".req b").html(countData.req).end()
     .find(".res b").html(countData.res).end()
     .find(".inproc b").html(countData.inproc).end()
     .find(".errors_req b").html(countData.errors_req).end()
     .find(".errors_resp b").html(countData.errors_resp).end()
     .find(".ended_req b").html(countData.ended_req).end();
  	doReport(timeData);
  });
  ...
  this.socket.on('error', function(message) {
      $("#stats")
     .find(".error label").html(message.node + ' '+ message.error).end();
  });
