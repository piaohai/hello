...
function updateGraph(chart,targetName) {
	...
	if (!main) {
		var _dom = $("#avg_template").clone().attr('id',targetName+'div').find('.screen-label').html(targetName+' Response Time Graph').end().find('.cgraphdiv').attr('id',targetName).end();
		$("#graph_div").append(_dom);
		main = document.getElementById(targetName);
		$(targetName).find('.closebtn').click(function(){
			alert('234');
		});
	};
	if (graphs[chart.uid]) {
