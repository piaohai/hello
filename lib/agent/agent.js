...
Agent.prototype = {
    ...
    run:function(message){
        var agent = this;
        var count = message.maxuser;
        agent.log.info(this.nodeId + ' run ' + count + ' robots ');
        monitor.clear();
        this.robots = {};
        for (var rid=0;rid<count;rid++) {
           var robot = new Robot(agent.conf,rid,agent.script);
           this.robots[rid]= robot;
           robot.on('error',function(error){
             agent.socket.emit('error',error);
           });
       };
       __.each(agent.robots,function(ele){ele.run();});
       setInterval(function(){
          ...
          if (__.size(timeData)>0){
            var stats = {req:0, res:0, inproc: 0, errors_req: 0, errors_resp: 0, ended_req: 0,};
             __.each(agent.robots,function(ele){
