...
var Actor = function(conf,aid) {
  ...
  self.on('start',function(action,reqId){
    monitor.beginTime(action,self.id,reqId,Date.now());
  });
  self.on('end',function(action,reqId){
    monitor.endTime(action,self.id,reqId,Date.now());
  });
  self.on('incr',function(action,reqId){
    monitor.incr(action);
  });
  self.on('decr',function(action,reqId){
    monitor.decr(action);
  });
};
